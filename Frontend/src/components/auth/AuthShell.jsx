...
export function AuthShell({ title, subtitle, children, contentWidth = "md" }) {
  const contentWidthClass = contentWidth === "lg" ? "max-w-lg" : "max-w-md";

  return (
    <main className="app-shell relative min-h-[100dvh] overflow-x-hidden px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
      <motion.div
      ...
      <div className="relative mx-auto flex min-h-[calc(100dvh-1.5rem)] w-full max-w-6xl items-center sm:min-h-[calc(100dvh-2rem)]">
        <motion.section
          className="gradient-border grid w-full overflow-hidden rounded-[1.5rem] glass-elevated md:max-h-[calc(100dvh-2rem)] md:grid-cols-[0.95fr_1.05fr] lg:rounded-[2rem]"
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: "easeOut" }}
        >
          <div className="relative hidden flex-col justify-between overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-8 text-white md:flex lg:p-10">
            ...
            <div className="relative">
              ...
              <h1 className="mt-6 max-w-sm text-3xl font-bold leading-tight tracking-tight lg:text-4xl">
          ...
          <div className="flex min-h-0 items-center justify-center px-5 py-6 sm:px-8 md:py-7 lg:px-10">
            <motion.div
              className={`w-full ${contentWidthClass}`}
